  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6 lg:p-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row animate-slide-up">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-3" /><input type="text" placeholder="Buscar guías por título o contenido…" value={search} onChange={e => setSearch(e.target.value)} className="w-full rounded-xl border border-teal/12 bg-bg-card/60 backdrop-blur-xl py-2.5 pl-10 pr-4 text-sm text-text-1 placeholder:text-text-3 focus:border-teal/50 focus:outline-none focus:ring-2 focus:ring-blue/20" /></div>
        <div className="flex items-center gap-2"><Filter className="h-4 w-4 text-text-2" /><select value={specialty} onChange={e => setSpecialty(e.target.value)} className="rounded-xl border border-teal/12 bg-bg-card/60 backdrop-blur-xl px-3 py-2.5 text-sm text-text-1 focus:border-teal/50 focus:outline-none cursor-pointer">{SPECIALTIES.map(s => <option key={s} value={s} className="bg-bg-card">{s}</option>)}</select></div>
      </div>

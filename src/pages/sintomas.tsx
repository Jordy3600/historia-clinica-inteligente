...
export default function SintomasPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6 lg:p-8">
      ...
      <div className="relative animate-slide-up"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-3" /><input type="text" placeholder="Buscar síntoma…" value={search} onChange={e => setSearch(e.target.value)} className="w-full rounded-xl border border-border bg-bg-card py-2.5 pl-10 pr-4 text-sm text-text-1 placeholder:text-text-3 focus:border-blue/50 focus:outline-none focus:ring-2 focus:ring-blue/20" /></div>
